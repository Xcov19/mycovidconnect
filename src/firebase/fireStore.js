...
class FireStore {
    ...
    static fetchCityData(cityName, mapCallback) {
        ...
        (async () => {
          ...
          const querySnapshot = await FireStore.#DB.collection(cityNameCollectionPath).get();
          // Get list of records objects.
          querySnapshot.forEach((docRef, index) => {
            const docData = docRef.data();
            const promise = new Promise(resolve => resolve(JSON.parse(docData.records)));
          ...
          const promiseData = hospitalRecordsResults.map(async (hospitalRecords) => {
            if (!!hospitalRecords) {
